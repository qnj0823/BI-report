<template>
    ...
    <el-dialog :title="!dataForm.id ? '新增-ADD' : '修改-EDITE'" :close-on-click-modal="false" :visible.sync="visible">
        ...
        <el-form :model="dataForm" ref="dataForm" @keyup.enter.native="dataSubmit()" label-width="160px">
            ...
            <el-form-item label="物流站点编码">
                <el-input v-model="dataForm.wlSiteCode" style="width: 100%;" placeholder="光明站点code">
                </el-input>
            </el-form-item>
            <el-form-item label="物流站点">
                <el-input v-model="dataForm.wlSiteName" style="width: 100%;" placeholder="光明站点code">
                </el-input>
            </el-form-item>
            <el-form-item label="产品名称(NCC)" prop="factoryProductName">
                <el-input v-model="dataForm.factoryProductName" style="width: 100%;" placeholder="光明产品名称">
                </el-input>
            </el-form-item>
            <el-form-item label="产品编号(NCC)" prop="factoryProductCode">
            ...
            <el-form-item label="大小月" prop="sheets">
                <el-input v-model="dataForm.dates" style="width: 100%;" placeholder="大小月">
                </el-input>
            </el-form-item>
        </el-form>
    ...
    methods: {
        ...
        dataSubmit() {
            this.$refs['dataForm'].validate((valid) => {
                if (!this.dataForm.id) {
                    api.wlFacteproaddTwoApi(this.dataForm).then(res => {
                        // TODO 保存数据
                        this.$emit('refreshDataList')
                        this.$emit('close')
                    ...
                    api.wlFacteproupdateTwoApi(this.dataForm).then(res => {
                        // TODO 保存数据
                        this.$emit('refreshDataList')
                        this.$emit('close')
