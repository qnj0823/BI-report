 ...
 */
export function exportExcelzj(tableList, date, fileName, gxtableList) {
  ...
  const labelText = `浙江${month}.${day}到货明细`; // 如果没有选择日期，显示默认文本 
  ...
  mergeDuplicateInFirstTwoRowszj(ws)
  styleTotalRowsZJ(ws)
  ...
  // 将工作表添加到工作簿中  
  XLSX.utils.book_append_sheet(wb, ws, '浙江物流汇总表');
...
// 行合并 - 针对B列为"浙江总计"的行进行合并
function mergeDuplicateInFirstTwoRowszj(ws) {
  ...
  for (let r = 0; r < rowCount; r++) {
    const cellB = XLSX.utils.encode_cell({ c: 1, r: r }); // B列是第1列(0-based)
    if (ws[cellB] && ws[cellB].v === "浙江总计") {
      targetRows.push(r);
    }
  }
  ...
  for (const row of targetRows) {
    // 只处理前三列（0-2列）
    for (let col = 0; col < 4; col++) {  // 只循环到第2列
...
// 总计行样式 - 将包含"总计"的行背景设为蓝色
function styleTotalRowsZJ(ws) {
  // 定义样式
...
function setExcelStyle(data) {
  ...
  data['!cols'] = [
    ...
    { wpx: 50.28 }, { wpx: 50.28 }, { wpx: 50.28 }, { wpx: 50.28 },
    { wpx: 50.28 }, { wpx: 50.28 }, { wpx: 50.28 }, { wpx: 50.28 }, { wpx: 50.28 }, { wpx: 50.28 }, { wpx: 50.28 }, { wpx: 50.28 }, { wpx: 50.28 }, { wpx: 50.28 }, { wpx: 50.28 }, { wpx: 50.28 }, { wpx: 50.28 }, { wpx: 380 } // 最后一列宽度为0（隐藏列）
  ];
