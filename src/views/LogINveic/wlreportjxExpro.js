 ...
 */
export function exportExceljx(tableList, date, fileName, tableList1, tableList3) {

  const selectedDate = new Date(date);
  const month = selectedDate.getMonth() + 1; // 获取月份（0-11，需要加 1）
  const year = selectedDate.getFullYear(); // 获取年份  
  const day = String(selectedDate.getDate()).padStart(2, '0');
  const labelText = `江西区域到货明细表--截止${year}年${month}月${day}日`; // 如果没有选择日期，显示默认文本 
  ...
  const tableTitle = ['区域', '大LOOK', '小LOOK', '健能', `LOOK优选`, '大白桃', `小白桃`, '大清新健爽（橙）', `小清新健爽（橙）`, `大0糖0脂`, `小0糖0脂`, `小原味戴永红`,
    `小原味绿叶水果`, `新鲜牧场`, `180噜渴(白)`, `180噜渴(红)`, `合计`]

  tableData.push(tableTitle)
  tableList.forEach(item => {
    tableData.push([item.wlSiteName || '', item.box1520100001 || '', item.box1520100002 || '', item.box1520100004 || '', item
      .box1520100008 || '', item.box1520100010 || '', item.box1520100009 || '', item.box1520100014 || '', item.box1520100015 || '', item.box1520100017 || '', item.box1520100016 || '',
    item.box1520100020 || '', item.box1520100021 || '', item.box1520110069 || '', item.box1520100025 || '', item.box1520100026 || '', item.sum || ''
    ])
  })
  ...
  // 准备第二个工作表的数据  
  const tableData3 = [
    ['区域', '大LOOK', '小LOOK', '健能', `LOOK优选`, '大白桃', `小白桃`, '大清新健爽（橙）', `小清新健爽（橙）`, `大0糖0脂`, `小0糖0脂`, `小原味戴永红`,
      `小原味绿叶水果`, `新鲜牧场`, `180噜渴(白)`, `180噜渴(红)`, `合计`]
  ];
  tableList3.forEach(item => {
    tableData3.push([item.vcol2_name || '', item.box1520100001 || '', item.box1520100002 || '', item.box1520100004 || '', item
      .box1520100008 || '', item.box1520100010 || '', item.box1520100009 || '', item.box1520100014 || '', item.box1520100015 || '', item.box1520100017 || '', item.box1520100016 || '',
    item.box1520100020 || '', item.box1520100021 || '', item.box1520110069 || '', item.box1520100025 || '', item.box1520100026 || '', item.sum || ''
    ])
  })
  ...
  // 创建第二个工作表  
  const ws3 = XLSX.utils.aoa_to_sheet(tableData3);
  ...
  XLSX.utils.book_append_sheet(wb, ws3, '九江中站');
...
function setExcelStyle(data) {
  ...
  // 2. 初始化列宽设置
  data['!cols'] = [
    { wpx: 130 }, { wpx: 100 }, { wpx: 100 }, { wpx: 80 },
