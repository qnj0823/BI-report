...
export function exportExcel(tableList, fileName) {
  let tableData = [
    ['战区', '销售区域/省区', '分公司/办事处', '客户营业执照', '站点', '一级账号', '下级账号', '下级站点']
  ]
  tableList.forEach(item => {
    tableData.push([item.levelName1 || '', item.levelName2 || '', item.levelName3 || '', item.custName || '', item
      .cuName || '', item.username || '', item.twousername || '', item.twocuname || ''
    ])
  })
