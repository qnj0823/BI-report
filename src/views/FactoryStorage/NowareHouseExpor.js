...
export function exportExcel(tableList, fileName) {
  ...
  let tableData = [
    ['战区', '销售区域/省区', '分公司/办事处', '客户营业执照', '客户站点', '订单总数', '入库数', '入库百分比', '入库差异']
  ]
  // tableData.push(tablebop)
  tableList.forEach(item => {
    tableData.push([item.levelName1 || '', item.levelName2 || '', item.levelName3 || '', item.cust_name || '', item.cu_name || '', item.total_count || '', item
      .mqty || '', item.rate || '', item.diff || ''
    ])
  })
