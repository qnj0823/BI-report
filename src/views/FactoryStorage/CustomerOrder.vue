<template>
    <div class='order'>
        ...
        <el-table class="table" ref="table" border :data="currentData" v-loading="dataListLoading"
            ...
            <el-table-column :show-overflow-tooltip="true"  width="250"  align="center" prop="cName" label="公司名称" />
            <el-table-column :show-overflow-tooltip="true" align="center" prop="cSiteName" label="客户站点" />
            <el-table-column :show-overflow-tooltip="true" align="center" prop="username" label="账号" />
...
export default {
    ...
    mounted() {
        const orderList = sessionStorage.getItem('order');
        if (orderList) {
            const orderList1 = JSON.parse(orderList);
            this.dataList = orderList1
            this.currentData = {
                ...this.dataList
            };
    ...
    methods: {
        exportData() {
            this.$confirm('是否导出表格数据到Excel?', '提示', {
                ...
                exportExcel(this.dataList, '客户站点订单表.xlsx')
        ...
        getdataList() {
            ...
            api.PtslDetailsone(this.dataForm).then(res => {
                this.dataList = res
                ...
                this.sizeChangeHandle(this.pageSize);
                sessionStorage.setItem('order', JSON.stringify(this.dataList));
