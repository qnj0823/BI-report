...
export function exportExcel(tableList, fileName) {
  ...
  let tableData = [
    ['开始时间','截至时间', '战区', '销售区域/省区', '分公司/办事处', '客户营业执照', '客户站点', '订单数量', '入库数量', '入库百分比']
  ]
  // tableData.push(tablebop)
  tableList.forEach(item => {
    tableData.push([item.statTime || '',item.endTime || '',item.levelName1 || '', item.levelName2 || '', item.levelName3 || '', item.cust_name || '', item
      .cu_name || '', item.total_count || '', item.mqty || '', item.rate || ''
    ])
  })
