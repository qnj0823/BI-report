<template>
    <div class='StorageDetailss'>
        ...
        <el-table border ref="table" v-loading="dataListLoading" :data="currentData" style="width: 100%;">
            ...
            <el-table-column :show-overflow-tooltip="true" align="center" prop="not_yet" label="未入库数" />
        </el-table>
...
export default {
    ...
    methods: {
        ...
        getDataList() {
            ...
            //     this.dataForm.p_vouchdateend = this.$route.query.endDate
            // }
            api.APIptsinoutdetails(this.dataForm).then(res => {
                ...
                this.dataList = this.groupAndSum(this.dataList)
        ...
        groupAndSum(data) {
            ...
            const grouped = data.reduce((acc, item) => {
                ...
                acc[key].total_count += Number(item.total_count) || 0;
                acc[key].mqty += Number(item.mqty) || 0;
            ...
            const result = Object.values(grouped).flatMap(group => [
                ...
                { // 合计行
                    ...
                    total_count: group.total_count.toString(),
                    mqty: group.mqty.toString(),
                    po_date: "合计",
                    pname: "合计"
                }
            ]);
        ...
        exportData() {
            this.$confirm('是否导出表格数据到Excel?', '提示', {
                ...
                exportExcel(this.dataList, '入库明细表.xlsx')
