...
export function exportExcel(tableList, fileName) {
  ...
  let tableData = [
    ['销售区域','分公司/办事处', '客户名称','PTS帐号', '站点名称', '日期', '产品名称', '订单数', '入库数', '未入库数']
  ]
  // tableData.push(tablebop)
  tableList.forEach(item => {
    tableData.push([item.levelName2|| '',item.levelName3|| '',item.cust_name || '',item.username || '', item.cu_name || '', item.po_date || '', item.pname || '', item.total_count || '', item
      .mqty || '', item.not_yet || ''
    ])
  })
...
function setExcelStyle(data) {
  ...
  data['!cols'] = [
    ...
    { wpx: 150 }, { wpx: 100 }, { wpx: 300 }, { wpx: 90 },
    // 最后一列宽度为0（隐藏列）
  ];
