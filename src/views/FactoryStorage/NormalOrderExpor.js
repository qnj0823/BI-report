...
export function exportExcel(tableList, fileName) {
  let tableData = [
    ['战区', '销售区域/省区', '分公司/办事处', '客户营业执照', '客户站点', '产品', '数量','订单日期','出库日期']
  ]
  tableList.forEach(item => {
    tableData.push([item.levelName1 || '', item.levelName2 || '', item.levelName3 || '', item.cName || '', item
      .cSiteName || '', item.cProductName || '', item.box || '', item.vouchdate || '', item.dbizdate || ''
    ])
  })
