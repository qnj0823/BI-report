...
export function exportExcel(tableList, fileName) {
  let tableData = [
    ['分公司办事处', '低温系列','常温系列','果汁系列','椰子牛乳系列','椰子汁系列','其它', '小计']
  ]
  tableList.forEach(item => {
    tableData.push([item.ocustomerclassName || '', item.lowSeries || '', item.NorSeries || '', item.FruitSeries || '',item.YzSeries || '',item.YzzSeries || '',item.QtSeries || '',item.sum || ''
    ])
  })
