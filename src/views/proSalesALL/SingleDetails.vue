    ...
    <div class='details'>
        ...
        <el-table class="table" ref="table" border :data="currentData" v-loading="dataListLoading"
            ...
            <el-table-column fixed :show-overflow-tooltip="true" align="center" width="200" prop="cSiteName"
                label="站点" />
            <el-table-column fixed :show-overflow-tooltip="true" align="center" width="200" prop="cname" label="客户名称" />
            ...
            <el-table-column :show-overflow-tooltip="true" align="center" prop="lowSeries" label="新品">
                ...
                <el-table-column :show-overflow-tooltip="true" align="center" prop="yzs380" width="120"
                label="380椰子水" />
        ...
        <el-pagination style="width: 80%; margin: 0 auto; " @size-change="sizeChangeHandle" ref="pagination"
            ...
            :page-size="pageSize" layout="total, sizes, prev, pager, next, jumper" :total="resultArray.length">
        </el-pagination>
    </div>
</template>
...
export default {
    ...
    mounted() {
        this.p_orgname = this.$route.query.param1;
    ...
    methods: {
        exportData() {
            this.$confirm('是否导出表格数据到Excel?', '提示', {
                ...
                exportExcel(this.resultArray, '单品销量表-站点.xlsx')

            })
        },
        getDataList() {
            console.log(this.p_orgname)
            this.dataListLoading = true;
            api.productonesellsite(this.dataForm).then(res => {
                this.dataList = res;

                this.dataList = this.dataList.filter(item =>
                    (item.nAME && item.nAME.toLowerCase().includes(this.p_orgname))
                ...
                console.log(this.dataList, 'dataList999')

                this.dataList = Object.values(this.dataList.reduce((acc, item) => {
                    const key = `${item.cProductCode}-${item.cSiteName}-${item.ocustomerClass_name}-${item.sqname}-${item.cname}-${item.nAME} `; // 生成唯一键  
                    if (!acc[key]) {
                        // 如果不存在该键，则创建一个新的对象  
                        acc[key] = {
                ...
                // 按照共同字段进行合并  
                this.dataList.forEach(item => {
                    const key = `${item.cSiteName}|${item.cname}|${item.nAME}|${item.ocustomerClass_name}|${item.sqname}`;
                    ...
                    Object.keys(item).forEach(field => {
                        if (field !== 'cSiteName' && field !== 'cname' && field !== 'nAME' &&
                            field !== 'ocustomerClass_name' && field !== 'sqname') {
                            aggregatedData[key][field] =
                                (aggregatedData[key][field] || 0) + parseFloat(item[field]);
                        }
                    });
                });

                // 将合并后的数据转换回数组  
                const addData = Object.values(aggregatedData);

                //计算小计
                this.resultArray = addData.map(obj => {
                    // 计算非指定字段的和  
                ...
                this.addSummaryToDataArray(this.resultArray);
        ...
        // 计算并添加合计对象  
        addSummaryToDataArray(dataArray) {
            // 计算并添加合计对象  
            const sumObject = dataArray.reduce((acc, obj) => {
                Object.keys(obj).forEach(key => {
                    // 过滤出不需要的字段  
                    if (!["cSiteName", "cname", "nAME", "ocustomerClass_name", "sqname"].includes(key)) {
                        // 转换为数字并累计和  
        ...
        sizeChangeHandle(val) {
            this.pageSize = val;
            this.currentPage = 1;
            this.currentData = this.resultArray.slice((this.currentPage - 1) * this.pageSize, this.currentPage * this
                .pageSize);
        },
        // 当前页
        handleCurrentChange(val) {
            ...
            this.currentData = this.resultArray.slice((val - 1) * this.pageSize, val * this.pageSize);
        },
