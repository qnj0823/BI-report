...
export function exportExcel(tableList, fileName) {
  let tableData = [
    ['分公司/办事处', '老品','老品百分比','新品','新品百分比', '其它', '其它百分比', '小计']
  ]
  tableList.forEach(item => {
    tableData.push([item.ocustomerClass_name || '', item.oldbox || '', item.oldrate || '', item.newbox || '',item.newrate || '',item.otherbox || '',item.otherrate || '',item.sum || ''
    ])
  })
