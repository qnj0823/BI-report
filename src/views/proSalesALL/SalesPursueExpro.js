...
export function exportExcel(tableList, fileName) {
  let tableData = [
    ['项目组', '本月挑战目标', '今日报单（全品）', '当月累计报单', '当月累计报单', '当月累计报单', '当月累计报单', '当月累计报单', '当月累计报单', 
      '当月累计报单（全品）', '全品完成率', '全品累计增幅'
    ]
  ]
  const tablebop = [ '项目组', '本月挑战目标', '今日报单（全品）', '老品', '老品', '新品（标准件）', '新品（标准件）', '新品（标准件）',
    '新品（标准件）','当月累计报单（全品）', '全品完成率', '全品累计增幅'
  ]
  const tableTop = [ '项目组', '本月挑战目标', '今日报单（全品）', '低温系列+330/310', '同比增幅', '常温系列', '1L椰子牛乳', '椰子汁系列',
    '果汁系列','当月累计报单（全品）', '全品完成率', '全品累计增幅'
  ]
  tableData.push(tablebop)
  tableData.push(tableTop)
  tableList.forEach(item => {
    tableData.push([item.ocustomerClass_name || '', item.goalvalue || '', item.sumallorder_today || '', item.sumLookLowNum_leiji || '', item.sumyuanweinumLeiji || '', item
      .sumLookNormalNum_leiji || '', item.sumYeZiNum_leiji || '', item.sumYeZiZhi_leiji || '', item.sumGuoZhiNum_leiji || '', item.sum_leiji_all || '', item.completion_rate || '',
      item.todaynum || ''
    ])
  })
