...
export function exportExcel(tableList, fileName) {
  let tableData = [
    ['销售区域/省区','分公司办事处', '低温系列','常温系列','果汁系列','椰子牛乳系列','椰子汁系列','其它', '小计']
  ]
  tableList.forEach(item => {
    tableData.push([ item.sqname || '', item.ocustomerClass_name || '', item.lowbox || '', item.nowbox || '', item.Frubox || '',item.yznlbox || '',item.yzzbox || '',item.qtbox || '',item.sum || ''
    ])
  })
