...
export function exportExcel(tableList, fileName) {
  let tableData = [
    ['产品名称', '简称', '产生日期', '生产批次', '单位', '库存数量','库存小计']
  ]
  tableList.forEach(item => {
    tableData.push([item.skudescr1 || '', item.simplename || '', item.lotatt01 || '', item.lotatt04 || '', item
      .item || '', item.box || '',item.sum || ''
    ])
  })
