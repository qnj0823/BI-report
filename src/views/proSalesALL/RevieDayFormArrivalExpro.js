...
export function exportExcel(tableList, fileName) {
  let tableData = [
    ['战区','省区', '分公司/办事处', '客户营业执照', '站点','报单时间','到货时间', '大原味', '小原味', '优选原味', '小原味（戴永红定制）', '小原味（绿叶定制）', '大白桃', '小白桃',
  ...
  tableList.forEach(item => {
    tableData.push([item.nAME || '', item.sqname || '',item.ocustomerClass_name || '',item.cname || '',item.cSiteName || '',item.vouchdate || '',item.arrivaldate || '', item.bigyuanwei || '', item.smallyuanwei || '', item.yxyuanwei || '', item.xyuanweidyh || '', item
