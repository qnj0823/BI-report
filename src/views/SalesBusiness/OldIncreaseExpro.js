...
export function exportExcel(tableList, date, fileName) {
  ...
  const labelText = `截止${year}年${month}月${day}日全国老品增幅情况表`; // 如果没有选择日期，显示默认文本 

  let tablelable = [[`${labelText}`, `${labelText}`, `${labelText}`, `${labelText}`, `${labelText}`, `${labelText}`, `${labelText}`, `${labelText}`, `${labelText}`]]

  let tableData = [
    '战区', `上年${month}月销量累计`, `本年${month}月销量累计`, '同比完成率(本月)', '差额件数', '上年截止当日销量累计', '本年截止当日销量累计', '同比完成率(当日)', '差额件数'
  ...
  tableList.forEach(item => {
    tablelable.push([item.orgname || '', item.lastlowSeries || '', item.lowSeries || '', item.MonthRate || '', item.mothdiff || '', item
      .last_year_low_num || '', item.low_num || '', item.LastRate || '', item.lastdiff || ''
...
function setExcelStyle(data) {
  ...
  for (let key in data) {
    if (data[key].constructor === Object) {
      ...
      data['!cols'] = [
        { wpx: 120 }, { wpx: 55 }, { wpx: 55 }, { wpx: 70 },
        { wpx: 80 }, { wpx: 80 }, { wpx: 80 },
        { wpx: 70 }, { wpx: 60 }, { wpx: 60 }, { wpx: 70 },
