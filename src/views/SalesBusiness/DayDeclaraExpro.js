...
export function exportExcel(tableList, fileName) {
  let tableData = [
    ['战区', '单位体', '客户站点', '本月目标', '今日报单（自然件）', '今日报单（自然件）', '今日报单（自然件）', '今日报单（自然件）', '今日报单（自然件）', 
      '今日报单（自然件）', '本单任务', '本单差额', '本月累计完成', '本月差额', '本月完成率'
    ]
  ]
  const tableTop = [ '战区', '单位体', '客户站点', '本月目标', '低温系列', '常温系列', '椰子牛乳系列', '椰子汁系列',
    '果汁系列','本单合计完成','本单任务', '本单差额', '本月累计完成','本月差额','本月完成率'
  ]
  ...
  tableList.forEach(item => {
    tableData.push([item.name || '', item.bigyuanwei || '', item.smallyuanwei || '', item.yxyuanwei || '', item.xyuanweidyh || '', item
      .xyuanweily || '', item.dbt || '', item.xbt || '', item.dqx || '', item.xqx || '',item.qinxi450 || '', item.djs || 0,
      item.xjs || 0, item.dlt || 0,item.xlt || 0,item.lsjs || 0,item.jn || 0,item.jh || 0,item.yz330|| 0,item.yz310|| 0
    ])
  })
