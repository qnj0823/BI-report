...
export function exportExcel(tableList, date, fileName) {
  ...
  const labelText = `${year}年${month}月全国各战区 完成进度---截止到${date}`; // 如果没有选择日期，显示默认文本 
  ...
  const tableTitle = ['当月完成率排名', '战区',  '本月任务', `${month}月已完成`, `${month}月已完成`, `${month}月已完成`, `${month}月已完成`, `${month}月已完成`,
    `${month}月已完成`, `${month}月已完成`, '本月差额', '本月完成率', `${month}月全品同比增幅`, '同比差异件数', `1-${month}月完成`, `1-${month}月完成`, `1-${month}月完成`
  ]
  const tablebop = ['当月完成率排名', '战区',  '本月任务', '老品', '老品', '新品（自然件）', '新品（自然件）',
    '新品（自然件）', '新品（自然件）', '新品（自然件）', '本月差额', '本月完成率', `${month}月全品同比增幅`, '同比差异件数', `1-${month}月同比老品增幅`, `1-${month}月全品同比增幅`, `1-${month}月同比差异件数`
  ]
  const tableTop = ['当月完成率排名', '战区',  '本月任务', '低温系列', '同比增幅', '常温系列', '椰子牛乳系列',
  ...
  tableList.forEach(item => {
    tableData.push([item.Rank || '', item.nAME || '', item.cur_task || '', item.lowSeries || '', item.lowRate || '', item
      .NorSeries || '', item.YzSeries || '', item.YzzSeries || '', item.FruitSeries || '', item.sum || '', item.curdiff || '',item.currate || '',item.sumRate || '',item.diff || '',item.LowyearRate || '',item.totalyearRate || '',item.diffyear || ''
...
function styleSpecificRows(ws) {  
  ...
  // 从第五行到第十五行（E5到E15）的样式应用  
  for (let row = 4; row <= 14; row++) { // 4对应第五行，14对应第十五行  
    const cellAddress = XLSX.utils.encode_cell({  
      c: 4, // E列的索引是4  
      r: row  
    });  
