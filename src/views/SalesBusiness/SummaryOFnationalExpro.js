...
export function exportExcel(tableList, date, fileName) {
  ...
  const labelText = `${year}年全国${month}月 全品目标跟进汇总表`; // 如果没有选择日期，显示默认文本 
  ...
  const tableTitle = ['战区内增幅排名(老品)', '战区', '单位体', '总经理', `${month}月预算目标`, `${month}月预算目标`, `${month}月预算目标`, `截至${day}号实际达成(自然件)`, `截至${day}号实际达成(自然件)`,
    `截至${day}号实际达成(自然件)`, `截至${day}号实际达成(自然件)`, `截至${day}号实际达成(自然件)`, '预算目标达成进度', '预算目标达成进度', '预算目标达成进度', '同比增幅(截止当日)', '同比增幅(截止当日)'
  ]
  const tablebop = ['战区内增幅排名(老品)', '战区', '单位体', '总经理', '低温系列(老品)', '其它系列(新品)', '合计',
    '低温系列', '常温系列', '果汁系列', '椰子汁系列', '椰子牛乳系列', '本月缺口', '余下天数', '余下每日目标', '老品', '累计増幅'
  ]
  ...
  tableList.forEach(item => {
    tableData.push([item.Rank || '', item.orgdqname || '', item.orgname || '', item.fzr || '', item.Moldmthbox || '', item
      .Mnewmthbox || '', item.Mallsum || '', item.lowBox || '', item.nowBox || '', item.FruBox || '', item.yzzBox || '', item.yznlBox || '', item.mouthDiff || '', item.remainingDays || '', item.remainingbox || '', item.oldbox || '', item.LowRate || ''
