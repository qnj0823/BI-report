<template>
    <div class='basicTeamsAndCompany'>
        ...
        <el-table ref="table" v-loading="dataListLoading" :data="currentData" style="width: 100%;">
            ...
            <el-table-column prop="teamsperson" align="center" label="班组负责人" />
...
import AddOrUpdate from './basicTeamsAndCompany-add-updata'
import {
    exportExcel
} from './basicTeamsAndCompanyExpro.js'
export default {
    components: {
        AddOrUpdate,
    },
    name: 'basicTeamsAndCompany-page',
    data() {
        return {
            msg: '(管理部)班组&单位体负责人数据表',
    ...
    methods: {
        exportData() {
            this.$confirm('是否导出表格数据到Excel?', '提示', {
                ...
                exportExcel(this.dataList.content, '班组/单位体负责人数据表.xlsx')
            })
        },
        //删除
        deleteHandle(id) {
            ...
            this.$confirm(`您确定对[ids=${ids.join(',')}]进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
                ...
                console.log('删除', ids);
                api.APITeamsAndDEL(ids).then(res => {
                    // TODO 处理删除
        ...
        uploadFile() {
            ...
            if (this.dataForm2.file) {
                ...
                api.APITeamsAndPORT(formData).then(res => {
                    this.getDataList()
        ...
        getDataList() {
            api.APITeamsAndCHECK(this.dataForm).then(res => {
                this.dataList = res
                // 过滤数据  
                ...
                this.dataList.content.sort((a, b) => {
                    const numA = parseInt(a.teamsno.replace("班组", ""), 10);
                    const numB = parseInt(b.teamsno.replace("班组", ""), 10);
                    return numA - numB;
                });
                this.currentData = {
