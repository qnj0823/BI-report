<template>
    ...
    <el-dialog :title="!dataForm.id ? '新增-ADD' : '修改-EDITE'" :close-on-click-modal="false" :visible.sync="visible">
        ...
        <el-form :model="dataForm" ref="dataForm" @keyup.enter.native="dataSubmit()" label-width="160px">
            ...
            <el-form-item label="椰子汁月目标" prop="fgsbscName">
                <el-input v-model="dataForm.yezizhimonthgoal" style="width: 25rem;" placeholder="椰子汁月目标">
                </el-input>
            </el-form-item>
            <el-form-item label="其他常温产品月目标" prop="fgsbscName">
                <el-input v-model="dataForm.othermonthgoal" style="width: 25rem;" placeholder="其他常温产品月目标">
                </el-input>
            </el-form-item>
        </el-form>
        ...
        dataSubmit() {
            this.$refs['dataForm'].validate((valid) => {
                if (!this.dataForm.id) {
                    ...
                    oapi.APIMonthGoalADD(this.dataForm).then(res => {
                        // TODO 保存数据
                        this.$emit('refreshDataList')
                        this.$emit('close')
                    ...
                    this.dataForm.months = this.currentMonth
                    oapi.APIMonthGoalEDIT(this.dataForm).then(res => {
                        // TODO 保存数据
                        this.$emit('refreshDataList')
                        this.$emit('close')
