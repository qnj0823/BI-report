<template>
    <div class='basicMonthRules'>
        <el-form :inline="true" style="margin-top: 10px;" :model="dataForm" @submit.native.prevent>
            <el-form-item>
                <el-input v-model="bullay" placeholder="模糊搜索" clearable style="width: 185px;" @keyup.enter.native="searchEnterFun()"
        ...
        <el-table ref="table" v-loading="dataListLoading" :data="currentData" style="width: 100%;">
            ...
            <el-table-column prop="days" align="center" label="本月多少号以后的报单计入下个月1号" />
...
import AddOrUpdate from './basicMonthRules-add-updata'
import {
    exportExcel
} from './basicMonthRulesExpro.js'
export default {
    components: {
        AddOrUpdate,
    },
    name: 'basicMonthRules-page',
    data() {
        return {
            msg: '(管理部)计入下月规则数据表',
    ...
    methods: {
        exportData() {
            this.$confirm('是否导出表格数据到Excel?', '提示', {
                ...
                exportExcel(this.dataList.content, '计入下月规则数据表.xlsx')
            })
        },
        //删除
        deleteHandle(id) {
            ...
            this.$confirm(`您确定对[ids=${ids.join(',')}]进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
                ...
                console.log('删除', ids);
                api.APIMonthRulesDEL(ids).then(res => {
                    // TODO 处理删除
        ...
        uploadFile() {
            ...
            if (this.dataForm2.file) {
                ...
                api.APIMonthRulesPORT(formData).then(res => {
                    this.getDataList()
        ...
        getDataList() {
            api.APIMonthRulesCHECK(this.dataForm).then(res => {
                this.dataList = res
                // 过滤数据  
