<template>
    ...
    <el-dialog :title="!dataForm.id ? '新增-ADD' : '修改-EDITE'" :close-on-click-modal="false" :visible.sync="visible">
        ...
        <el-form :model="dataForm" ref="dataForm" @keyup.enter.native="dataSubmit()" label-width="160px">
            ...
            <el-form-item label="计入下个月1号时间" prop="fgsbscName">
                <el-input v-model="dataForm.days" style="width: 25rem;" placeholder="请以2025/4/1这种形式">
                </el-input>
            </el-form-item>
        </el-form>
        ...
        dataSubmit() {
            this.$refs['dataForm'].validate((valid) => {
                if (!this.dataForm.id) {
                    ...
                    oapi.APIMonthRulesADD(this.dataForm).then(res => {
                        // TODO 保存数据
                        this.$emit('refreshDataList')
                        this.$emit('close')
                    ...
                    this.dataForm.months = this.currentMonth
                    oapi.APIMonthRulesEDIT(this.dataForm).then(res => {
                        // TODO 保存数据
                        this.$emit('refreshDataList')
                        this.$emit('close')
