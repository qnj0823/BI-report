<template>
    <div class='basicMonthGoal'>
        <el-form :inline="true" style="margin-top: 10px;" :model="dataForm" @submit.native.prevent>
            ...
            <el-form-item>
                ...
                <el-button size="mini" class="filter-item" type="warning" icon="el-icon-download"
                    @click="exportData">导出/下载模板</el-button>
            </el-form-item>
        </el-form>
        <el-form :inline="true" style="margin-top: 10px;" :model="dataForm" @submit.native.prevent>
            <el-form-item>
        ...
        <el-table ref="table" v-loading="dataListLoading" :data="currentData" style="width: 100%;">
            ...
            <el-table-column prop="yezizhimonthgoal" align="center" label="椰子汁月目标" />
            <el-table-column prop="othermonthgoal" align="center" label="其他常温产品月目标" />
...
import AddOrUpdate from './basicMonthGoal-add-updata'
import {
    exportExcel
} from './basicMonthGoalExpro.js'
export default {
    components: {
        AddOrUpdate,
    },
    name: 'basicMonthGoal-page',
    data() {
        return {
            msg: '(管理部)椰子汁&其他常温产品月目标数据表',
    ...
    methods: {
        exportData() {
            this.$confirm('是否导出表格数据到Excel?', '提示', {
                ...
                exportExcel(this.dataList.content, '椰子汁/其他常温产品月目标数据表.xlsx')
            })
        },
        //删除
        deleteHandle(id) {
            ...
            this.$confirm(`您确定对[ids=${ids.join(',')}]进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
                ...
                console.log('删除', ids);
                api.APIMonthGoalDEL(ids).then(res => {
                    // TODO 处理删除
        ...
        uploadFile() {
            ...
            if (this.dataForm2.file) {
                ...
                api.APIMonthGoalPORT(formData).then(res => {
                    this.getDataList()
        ...
        getDataList() {
            api.APIMonthGoalCHECK(this.dataForm).then(res => {
                this.dataList = res
                // 过滤数据  
