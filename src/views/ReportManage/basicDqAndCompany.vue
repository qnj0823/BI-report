<template>
    <div class='basicDqAndCompany'>
        <el-form :inline="true" style="margin-top: 10px;" :model="dataForm" @submit.native.prevent>
            <el-form-item>
                <el-input v-model="bullay" placeholder="模糊搜索" style="width: 185px;" clearable
                    @keyup.enter.native="searchEnterFun()" ref="searchInput"></el-input>
            </el-form-item>
            <el-form-item>
                <el-button class="filter-item" size="mini" type="success" icon="el-icon-search"
                    @click="getDataList">查询</el-button>
                <el-button class="filter-item" size="mini" type="success" @click="addOrUpdateHandle()">新增分类</el-button>
                <el-button size="mini" class="filter-item" type="warning" icon="el-icon-download"
                    @click="exportData">下载模板/导出</el-button>
            </el-form-item>
        </el-form>
        <el-form :inline="true" style="margin-top: 10px;" :model="dataForm" @submit.native.prevent>
            <el-form-item>
        ...
        <el-table ref="table" v-loading="dataListLoading" :data="currentData" style="width: 100%;">
            ...
            <el-table-column :show-overflow-tooltip="true" align="center" prop="dqperson" label="战区负责人" />
            <el-table-column prop="companyname" align="center" label="单位体" />
            <el-table-column prop="companyperson" align="center" label="负责人" />
...
import AddOrUpdate from './basicDqAndCompany-add-updata'
import {
    exportExcel
} from './basicDqAndCompanyExpro.js'
export default {
    components: {
        AddOrUpdate,
    },
    name: 'basicDqAndCompany-page',
    data() {
        return {
            msg: '(管理部)大区&单位体负责人数据表',
    ...
    methods: {
        exportData() {
            this.$confirm('是否导出表格数据到Excel?', '提示', {
                ...
                exportExcel(this.dataList.content, '战区单位体负责人数据表.xlsx')
            })
        },
        //删除
        deleteHandle(id) {
            ...
            this.$confirm(`您确定对[ids=${ids.join(',')}]进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
                ...
                console.log('删除', ids);
                api.APIDqAndCompanyDEL(ids).then(res => {
                    // TODO 处理删除
        ...
        uploadFile() {
            ...
            if (this.dataForm2.file) {
                ...
                api.APIDqAndCompanyMPORT(formData).then(res => {
                    this.getDataList()
        ...
        getDataList() {
            api.APIDqAndCompanyCHECK(this.dataForm).then(res => {
                this.dataList = res
                // 过滤数据  
