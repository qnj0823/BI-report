...
/**  
 * 导出数据到 Excel 文件，包含一个工作表。  
 * @param {Array} tableList - 第一个工作表的数据从表。  
 * @param {Array} dataList - 第一个工作表的表头数据从表。  
  
 * @param {String} fileName - 导出文件的名称。  
 */  
export function exportExcel(tableList, dataList,  fileName) {  
  // 准备第一个工作表的数据  
  const tableData = [
    [
  ...
  // 提取 itemcode  
  const itemCodes = dataList.map(item => item.itemcode);
  ...
  // 将工作表添加到工作簿中  
  XLSX.utils.book_append_sheet(wb, ws, '费用明细主表');  
