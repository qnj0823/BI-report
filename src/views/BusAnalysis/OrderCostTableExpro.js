...
export function exportExcel(tableList, fileName) {
  ...
  let tableData = [
    ['销售订单与采购成本表', '销售订单与采购成本表', '销售订单与采购成本表', '销售订单与采购成本表', '销售订单与采购成本表', '销售订单与采购成本表', '销售订单与采购成本表', '销售订单与采购成本表', '销售订单与采购成本表', '销售订单与采购成本表', '销售订单与采购成本表']
  ]
  const tableTop = ['战区', '销售区域/省区', '分公司/办事处', '客户名称', '站点', '订单日期', 'BIP订单号', 'NCC订单号', '订单数量', '订单总价', '采购价总价']
  tableData.push(tableTop)

  tableList.forEach(item => {
    tableData.push([item.levelName1 || '', item.levelName2 || '', item.levelName3 || '', item.cName || '', item.cSiteName || '', item
      .vouchdate || '', item.vsrccode || '', item.cOrderNo || '', item.box || '', item.fSaleCost || '', item.cost_price || ''
    ])
  })
...
function setExcelStyle(data) {
  ...
  for (let key in data) {
    if (data[key].constructor === Object) {
      ...
      data['!cols'] = [
        { wpx: 80 }, { wpx: 80 }, { wpx: 140 }, { wpx: 180 },
        { wpx: 130 }, { wpx: 70 }, { wpx: 140 },
        { wpx: 140 }, { wpx: 70 }, { wpx: 70 }, { wpx: 70 },
