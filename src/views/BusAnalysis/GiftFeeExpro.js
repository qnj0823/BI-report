 ...
 */  
export function exportExcel(tableList, tableList1, fileName) {  
  // 准备第一个工作表的数据  
  const tableData = [
    [
      '战区','销售区域/省区','分公司/办事处','客户名称','数量'
    ]
  ];

  tableList.forEach(item => {
    tableData.push([item.levelName1 || '', item.levelName2 || '', item.levelName3 || '', item.cName || '', item.boxs || ''
    ])
  })
  ...
  const tableData1 = [
    [
      '战区','销售区域/省区','分公司/办事处','客户名称','产品名称', '数量'
    ]
  ];
 

  tableList1.forEach(item => {
    tableData1.push([item.levelName1 || '', item.levelName2 || '', item.levelName3 || '', item.cName || '',  item.cProductName || '', item.box || ''
    ])
  })
  ...
  // 将工作表添加到工作簿中  
  XLSX.utils.book_append_sheet(wb, ws1, '赠品数量主表');  
  XLSX.utils.book_append_sheet(wb, ws2, '赠品数量从表');  
