...
export function exportExcel(tableList, fileName) {
  let tableData = [
    ['站点ID','战区','销售区域/省区', '分公司/办事处','客户名称','销量区域', '站点', '人力+运营']
  ]
  tableList.forEach(item => {
    tableData.push([item.dEF1 || '',item.levelName1 || '', item.levelName2 || '', item.pkCustclassName|| '', item.name || '',item.def5Name || '',item.def1Name || '',item.people || ''
    ])
  })
