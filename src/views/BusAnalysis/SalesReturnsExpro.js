...
export function exportExcel(tableList, fileName) {
  let tableData = [
    ['退货日期','销售组织','出库单号', '客户编码', '客户名称', '退货产品编码', '退货产品名称', '退货数量']
  ]
  tableList.forEach(item => {
    tableData.push([item.returnGoodsDate || '',item.saleOrg || '',item.orderNum || '', item.custCode || '', item.custName|| '', item.productCode || '', item.productName || '', item.productNum || ''
    ])
  })
