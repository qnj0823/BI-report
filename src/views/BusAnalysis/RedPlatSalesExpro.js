...
export function exportExcel(tableList, fileName) {
  let tablelable = [['红平台（订单执行情况统计表）', '红平台（订单执行情况统计表）', '红平台（订单执行情况统计表）', '红平台（订单执行情况统计表）', '红平台（订单执行情况统计表）', '红平台（订单执行情况统计表）', '红平台（订单执行情况统计表）', '红平台（订单执行情况统计表）', '红平台（订单执行情况统计表）', '红平台（订单执行情况统计表）', '红平台（订单执行情况统计表）', '红平台（订单执行情况统计表）', '红平台（订单执行情况统计表）', '红平台（订单执行情况统计表）', '红平台（订单执行情况统计表）', '红平台（订单执行情况统计表）']]

  const tableTop = ['单据日期', '销售组织', '订单号', '客户', '客户分类', '红平台名称', '站点名称',
    '商品分类', '商品编码', '商品名称', '订单销售数量', '销售单位', '含税单价', '返利分摊金额', '订单含税金额', '分摊前金额'
  ]

  tablelable.push(tableTop)

  tableList.forEach(item => {
    tablelable.push([item.vouchdate || '', item.salesOrgName || '', item.cOrderNo || '', item.cName || '', item.ocustomerClass_name || '', item
      .red_name || '', item.cSiteName || '', item.product_class_name || '', item.cProductCode || '', item.cProductName || '', item.box || '',
    item.cProductAuxUnitName || '', item.nqtorigtaxprice || '', item.norigdiscount || '', item.fPayMoney || '', item.costsubs || ''
...
function setExcelStyle(data) {
  ...
  for (let key in data) {
    if (data[key].constructor === Object) {
      ...
      data['!cols'] = [
        { wpx: 60 }, { wpx: 100 }, { wpx: 130 }, { wpx: 120 }, { wpx: 100 },
        { wpx: 100 }, { wpx: 100 }, { wpx: 70 }, { wpx: 70 }, { wpx: 150 },
        { wpx: 70 }, { wpx: 100 }, { wpx: 70 }, { wpx: 100 }, { wpx: 100 },
