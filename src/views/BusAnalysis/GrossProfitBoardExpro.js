...
export async function exportExcel(tableList, startDate, endDate, areas, fileName) {
  ...
  worksheet.columns = [
    ...
    { header: '6.20-8.20\n低温目标', key: 'goalnumtotal', width: 12, hidden: false },
    ...
    { header: '累计目标', key: 'ordersnumStage', width: 12, hidden: false },
  ...
  // 添加表头和数据
  const headers =['省区', '单位体', '客户/站点', '6.20-8.20\n低温目标', '今日完成', `今日完成`,'今日完成', `今日完成`,'今日完成', `今日完成`,`截止6.20今日累计完成`, `截止今日累计完成`, `截止今日累计完成`,
    `截止今日累计完成`, `截止今日累计完成`,`截止今日累计完成`];
  const subHeaders = ['省区', '单位体', '客户/站点', '6.20-8.20\n低温目标', `今日目标`,'look系列', `330/310`,'180宴席', `今日已完成`,`今日缺口\n(正为缺口)`, `累计目标`, `look系列`,`330/310`,
    `180宴席`, `累计完成`, `累计缺口\n(正为缺口)`];
...
function styleColumnsJandP(worksheet) {
  ...
  worksheet.eachRow((row, rowNumber) => {
    ...
    if (rowNumber >= 4) {
      ...
      // 获取J列和P列的单元格
      const jCell = row.getCell(10); // J列是第10列
