...
export function exportExcel(tableList, fileName) {
  let tableData = [
    ['销售区域', '销售区域', '实际订单/件', '实际订单/盒', '光明销售公司', '工厂供应链', '是否满产']
  ]
  tableList.forEach(item => {
    tableData.push([item.sname || '', item.iParentName || '', item.box|| '', item.piece || '', item.salesorgname || '', item.factoryname || '', item.isfull || ''
    ])
  })
