...
export default {
    ...
    created() {
        ...
        // 监听事件，触发时执行 getdataList
        EventBus.$on('triggerGetDataList', this.getdataList);

    },
    beforeDestroy() {
        // 避免内存泄漏
        EventBus.$off('triggerGetDataList', this.getdataList);
    },
    mounted() {
        // this.fetchAllData()
        this.getdataList()
    ...
    methods: {
        ...
        async getdataList() {
            ...
            this.labelText = `${year}年${month}月各单位体低温报单进度表--截止${month}月${day}日`;
            try {
                const [lowtempres, TeamsAnres, tempcurres, LowTempdata, montrhsinger, history] = await Promise.all([
                    ...
                    api.APIlowgetcurall(this.lowtempForm),//今日报单每半个小时计算一次
        ...
        async fetchAllData() {
            ...
            try {
                ...
                // // 2. 单独发起 history 请求，并立即监听其完成
                // const historyRequest = api.APIgetcurall(this.historyForm);

                // // 3. 等待 history 请求完成（不等待其他接口）
