...
import {
    exportExcel
} from './WorkUnitNormalExpro.js'
import * as api from '@/api/management/management.js'
export default {
    name: 'WorkUnitNormal-page',
    data() {
        return {
            msg: '班组单位体常温报单进度表',
    ...
    methods: {
        ...
        async fetchAllData() {
            ...
            this.labelText = `${year}年${month}月班组单位体常温报单进度表--截止${month}月${day}日`; // 如果没有选择日期，显示默认文本
            try {
                ...
                this.TeamsAnList.forEach(item => {
                    if (item && typeof item.box === 'number') {
                        item.box = parseFloat(item.box.toFixed(1));
                    } else if (item && typeof item.box === 'string') {
                        const num = parseFloat(item.box);
                        if (!isNaN(num)) {
                            item.box = parseFloat(num.toFixed(1));
                        }
                    }
                });
                ...
                console.log('班组/单位体常温目标数据表:', this.dqdaList)
                ...
                this.dataList = this.dataList.map(item => {
                    ...
                    for (const key in item) {
                        if (key === 'companycode' || key === 'companyid'|| key === 'companyname'|| key === 'companyperson'|| key === 'teamsno'|| key === 'teamsperson') {
                            // 保留这三个字段不变
        ...
        calculateDates() {
            ...
            this.labelText = `${this.year}年${this.month}月班组单位体常温报单进度表--截止${this.month}月${this.day}日`; // 如果没有选择日期，显示默认文本
        },
        formatDate(date) {
        ...
        exportData() {
            this.$confirm('是否导出表格数据到Excel?', '提示', {
                ...
                exportExcel(this.dataList, this.dataForm.p_vouchdateend, '班组单位体常温报单进度表.xlsx')
