...
export function exportExcel(tableList, date, fileName) {
  ...
  const labelText = `${year}年${month}月各班组单位体常温报单进度表--截止${month}月${day}日`; // 如果没有选择日期，显示默认文本 
  console.log(month, year)

  let tableData = [
    [`${labelText}`, `${labelText}`, `${labelText}`, `${labelText}`, `${labelText}`, `${labelText}`, `${labelText}`, `${labelText}`,
    `${labelText}`, `${labelText}`, `${labelText}`, `${labelText}`, `${labelText}`, `${labelText}`, `${labelText}`
    ]
  ]
  const tableTitle = ['班组', '单位体', '负责人', '今日报单(常温全品)', `${month}月目标\n(光明椰子汁)`, `${month}月目标(其它常温)`, `${month}月目标(常温全品)`, `${month}月累计-常温系列(标准件)`, `${month}月累计-常温系列(标准件)`,
    `${month}月累计-常温系列(标准件)`, `${month}月累计-常温系列(标准件)`, `${month}月累计-常温系列(标准件)`, `${month}月累计-常温系列(标准件)`, `${month}月差额(常温全品)`, `${month}月差额\n(光明椰子汁)`]
  const tablebop = ['班组', '单位体',  '负责人', '今日报单(常温全品)', `${month}月目标\n(光明椰子汁)`, `${month}月目标(其它常温)`,`${month}月目标(常温全品)`, `300常温`, `950常温`,
    `果汁系列`, `1L椰乳`, `椰子汁系列截止今日累计完成`, `常温系列截止今日累计完成`, `${month}月差额(常温全品)`, `${month}月差额\n(光明椰子汁)`]

  tableData.push(tableTitle)
  tableData.push(tablebop)
  tableList.forEach(item => {
    tableData.push([item.teamsno || '', item.companyname || '', item.companyperson || '', item.box || '', item
      .yzzgoal || '', item.nomalgoal || '', item.otherbox || '', item.box300 || '', item.box950 || '', item.gzbox || '',
    item.yzbox1l || '', item.yezibox || '', item.nomalTemp || '', item.nomaldiff || '', item.diffyzz || ''
    ])
  })
...
function styleRowsWithSubtotal(ws) {
  ...
  const lightGreenBackground = {
    fill: {
      fgColor: {
        rgb: "e2f0d9"  // RGB color code for light green
      }
    }
  };
...
function setExcelStyle(data) {
  ...
  data['!cols'] = [
    ...
    { wpx: 90 }, { wpx: 75 }, { wpx: 75 }, { wpx: 90 },
    { wpx: 90 }, { wpx: 90 }, { wpx: 90 }, { wpx: 110 }, { wpx: 100 }, { wpx: 75 }, { wpx: 90 } // 最后一列宽度为0（隐藏列）
  ];
