...
export function exportExcel(tableList, date, fileName) {
  ...
  const labelText = `${year}年${month}月班组低温报单进度表--截止${month}月${day}日`; // 如果没有选择日期，显示默认文本 
  ...
  const tableTitle = ['序号', '班组', '负责人', '今日报单\n(低温)', `${year - 1}年${month}月销售数据`,'低温增幅', `${month}月低温任务`,'本月单数', `本月平均\n每单任务`,`当天单数`, `${month}月累计-低温系列(到货数)`, `${month}月累计-低温系列(到货数)`,
    `${month}月累计-低温系列(到货数)`,`${month}月累计-低温系列(到货数)`, `${month}月截止\n今日差额`, `${month}月累计\n差额`, `${month}月低温同比增幅`, `${month}月\n完成率`]
  const tablebop = ['序号', '班组', '负责人', '今日报单\n(低温)', `${year - 1}年${month}月销售数据`,'低温增幅', `${month}月低温任务`,'本月单数', `本月平均\n每单任务`,`当天单数`, `LOOk系列`, `330/310系列`,`180系列`,
    `截止今日完成`, `${month}月截止\n今日差额`, `${month}月累计\n差额`, `${month}月低温同比增幅`, `${month}月\n完成率`]
...
function setExcelStyle(data) {
  ...
  // 2. 初始化列宽设置
  data['!cols'] = [
    { wpx: 90 }, { wpx: 80 }, { wpx: 90 }, { wpx: 90 },
    { wpx: 90 }, { wpx: 90 }, { wpx: 90 }, { wpx: 90 },
    { wpx: 90 }, { wpx: 90 }, { wpx: 90 }, { wpx: 100 }, { wpx: 90 }, { wpx: 90 } // 最后一列宽度为0（隐藏列）
  ];
