...
import {
    exportExcel
} from './WarZoneNormalExpro.js'
import * as api from '@/api/management/management.js'
export default {
    name: 'WarZoneNormal-page',
    data() {
        return {
            msg: '战区常温报单进度表',
    ...
    methods: {
        ...
        async fetchAllData() {
            ...
            this.labelText = `${year}年${month}月战区常温报单进度表--截止${month}月${day}日`; // 如果没有选择日期，显示默认文本
            try {
        ...
        calculateDates() {
            ...
            this.labelText = `${this.year}年${this.month}月战区常温报单进度表--截止${this.month}月${this.day}日`; // 如果没有选择日期，显示默认文本
        },
        formatDate(date) {
        ...
        exportData() {
            this.$confirm('是否导出表格数据到Excel?', '提示', {
                ...
                exportExcel(this.dataList, this.dataForm.p_vouchdateend, '战区常温报单进度表.xlsx')
