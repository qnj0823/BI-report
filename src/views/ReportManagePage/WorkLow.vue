...
export default {
    ...
    methods: {
        ...
        async getdataList() {
            ...
            this.labelText = `${year}年${month}月班组低温报单进度表--截止${month}月${day}日`; // 如果没有选择日期，显示默认文本
            try {
                ...
                this.dataList = this.dataList.map(item => {
                    ...
                    for (const key in item) {
                        if (key === 'Accmplish' || key === 'teamsno' || key === 'rate'|| key === 'teamsperson') {
                            // 保留这三个字段不变
        ...
        calculateCurrentYearDates() {
            ...
            this.labelText = `${this.year}年${this.month}月班组低温报单进度表--截止${this.month}月${this.day}日`; // 如果没有选择日期，显示默认文本
        },
        ...
        exportData() {
            this.$confirm('是否导出表格数据到Excel?', '提示', {
                ...
                exportExcel(this.dataList, this.dataForm.p_vouchdateend, '班组低温报单进度表.xlsx')
