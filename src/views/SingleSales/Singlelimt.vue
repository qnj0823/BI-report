<template>
    <div class='single'>
        ...
        <el-table class="table" ref="table" border :data="dataList" v-loading="dataListLoading"
            ...
            <el-table-column fixed :show-overflow-tooltip="true" align="center" prop="companyname" label="分公司/办事处" />
...
export default {
    ...
    methods: {
        ...
        getDataList() {
            ...
            api.productonesellcompany(this.dataForm).then(res => {
                this.dataList = res
                ...
                this.dataList = this.dataList.map(item => {
                    ...
                    const newItem = { companyname: item.companyname }; // 遍历对象的每个字段 
                    for (const key in item) {
                        if (key !== 'companyname') { // 排除 name 字段 
