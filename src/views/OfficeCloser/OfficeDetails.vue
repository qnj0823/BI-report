<template>
    <div class='OfficeDetails'>
        ...
        <el-table border ref="table" v-loading="dataListLoading" height="680" :data="currentData" style="width: 100%;">
            ...
            <el-table-column prop="goalnumtotal" width="80" align="center"  label="6.20-8.20低温目标" />
            <el-table-column :show-overflow-tooltip="true" align="center" prop="converBigPiece" :label="`今日完成`">
                ...
                <el-table-column :show-overflow-tooltip="true" align="center" prop="todaydiff" width="80" label="今日缺口(正为缺口)">
                </el-table-column>
            </el-table-column>
            <el-table-column :show-overflow-tooltip="true" align="center" prop="daydiff" label="截止6.20今日累计完成">
                ...
                <el-table-column :show-overflow-tooltip="true" align="center" prop="leijidiff" width="80" label="累计缺口(正为缺口)">
                </el-table-column>
            </el-table-column>
        </el-table>
    ...
    methods: {
        async getdataList() {
            ...
            try {
                ...
                //根据站点名称去重
                const uniqueArray = [...new Map(this.goalList.map(item => [item.sitename, item]))].map(([_, value]) => value);
                console.log(uniqueArray, 'uniqueArray')

                //根据单位体id去重的结果添加负责人
                this.dataList = this.dataList.map(dataItem => {
                    const matchedUniqueItem = uniqueArray.find(uniqueItem => uniqueItem.sitename === dataItem.cSiteName);
                ...
                console.log(dataStage, 'dataStage', filteredData2)
                this.dataList = this.dataList.map(dataItem => {
                    const matchedUniqueItem = dataStage.find(uniqueItem => uniqueItem.sitename === dataItem.cSiteName);
                ...
                this.dataList = this.dataList.map(dataItem => {
                    const matchedUniqueItem = filteredArray.find(uniqueItem => uniqueItem.sitename === dataItem.cSiteName);
                    if (matchedUniqueItem && matchedUniqueItem.sitename !== undefined) {
                        return { ...dataItem, goalnum: matchedUniqueItem.goalnum, mark: matchedUniqueItem.mark }; // 合并字段（不修改原对象）
                    }
                    return dataItem; // 无匹配则返回原对象
                });
        ...
        addSummaryObject(array) {
            ...
            const sumFields = [
                ...
                'leiji180box', 'leijiend', 'leijidiff'
            ];
