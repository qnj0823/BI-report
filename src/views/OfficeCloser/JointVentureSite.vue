...
import {
    exportExcel
} from './JointVentureSiteExpro.js'
import * as api from '@/api/frame/customer.js'
export default {
    name: 'JointVentureSite-page',
    data() {
        return {
            msg: 'JointVentureSite-page',
            dataForm: {
                p_vouchdatestart: '',
                p_vouchdateend: ''
            },
    ...
    methods: {
        ...
        async getdataList() {
            ...
            this.labelText = `${this.year}年全国 各合资/省平台 低温销售数据进度表(${month}月${day}日—${monthend}月${dayend}日)`;
            ...
            try {
                ...
                this.dataList = this.dataList.filter(item =>
                    item.type == "集团平台" || item.type == "集团合资公司"
                );
                //历史到今天的数据
                this.dataList.forEach(item => {
                ...
                this.dataList = this.dataList.map(dataItem => {
                    const matchedUniqueItem = uniqueArray.find(uniqueItem => uniqueItem.companyid === dataItem.companyid);
                ...
                const uniquezdArray = [
                    ...new Map(
                        this.goalList.map((item) => [
                            `${item.companyid}_${item.sitename}`, // 用组合键作为唯一标识
                            item,
                        ])
                    ).values(),
                ];
                ...
                this.dataList = this.dataList.map(dataItem => {
                    const matchedUniqueItem = mergedArray.find(uniqueItem => uniqueItem.companyid === dataItem.companyid);
                ...
                this.dataList = this.dataList.map(dataItem => {
                    const matchedUniqueItem = datesqStage.find(uniqueItem => uniqueItem.companyid === dataItem.companyid);
                    if (matchedUniqueItem && matchedUniqueItem.sqman !== undefined) {
                ...
                this.dataList = this.dataList.map(dataItem => {
                    const matchedUniqueItem = filtereddateArray.find(uniqueItem => uniqueItem.companyid === dataItem.companyid);
                    if (matchedUniqueItem && matchedUniqueItem.sqman !== undefined) {
                ...
                // this.dataList = this.dataList.filter(item => item.sqname !== "省平台总计合计");
                this.dataList.forEach(item => {
        ...
        exportData() {
            this.$confirm('是否导出表格数据到Excel?', '提示', {
                ...
                exportExcel(this.dataList, this.dataForm.p_vouchdatestart, this.dataForm.p_vouchdateend, '合资/省平台低温销售数据进度表.xlsx')
            })
        },
        // 获取今年的日期数据
        calculateCurrentYearDates() {
            ...
            this.labelText = `${this.year}年全国 各合资/省平台 低温销售数据进度表(${this.month}月${this.day}日—${this.month}月${this.day}日)`;
        },
        // 格式化日期
        formatDate(date) {
