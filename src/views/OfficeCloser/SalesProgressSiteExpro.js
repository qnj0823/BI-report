...
export async function exportExcel(tableList, startDate, endDate,month1,day1,monthend1,dayend1, fileName) {
  ...
  const labelText = `${year}年全国(各分公司)低温销售数据进度表(${month}月${day}日—${monthEnd}月${dayEnd}日)`;
  ...
  worksheet.columns = [
    ...
    { header: '累计缺口\n(正为缺口)', key: 'todaybox', width: 12, hidden: false },
...
function styleSubtotalRows(worksheet) {
  ...
  worksheet.eachRow((row, rowNumber) => {
    ...
    // 检查单元格是否有值且包含"全国"
    if (cell.value && typeof cell.value === 'string' && cell.value.includes('分公司总计')) {
      row.eachCell(cell => {
        cell.style = {
          fill: {
            type: 'pattern',
...
function styleDatabscRows(worksheet) {
 ...
 worksheet.eachRow({ includeEmpty: true }, (row, rowNumber) => {
   // 检查A列（或其他关键列）是否包含"分公司总计"
   const firstCellValue = row.getCell(2).value;
   if (firstCellValue && firstCellValue.toString().includes("分公司总计")) {
...
function styleColumnsJandP(worksheet) {
  ...
  worksheet.eachRow((row, rowNumber) => {
    ...
    if (rowNumber >= 4) {
      ...
      // 获取J列和P列的单元格
      const jCell = row.getCell(6); // J列是第10列
