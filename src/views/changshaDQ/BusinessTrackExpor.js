...
export function exportExcel(tableList, fileName) {
  let tableData = [
    ['销售区域/省区', '分公司/办事处', '当月目标', '当月销量','上月销量','当天报单','同比','环比']
  ]
  tableList.forEach(item => {
    tableData.push([item.sqname || '', item.companyname || '', item.goalvalue || '', item.box || '', item.lastbox || '',item.curbox || '', item.rate || '', item.ratio || ''])
  })
