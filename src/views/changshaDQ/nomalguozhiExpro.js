...
export function exportExcel(tableList, fileName) {
  let tableData = [
    ['销售区域/省区','分公司办事处','站点', '低温系列','常温系列','果汁系列','椰子牛乳系列','椰子汁系列','其它', '小计']
  ]
  tableList.forEach(item => {
    tableData.push([ item.sqname || '', item.ocustomerclassName || '',item.cSiteName || '', item.lowSeries || '', item.NorSeries || '', item.FruitSeries || '',item.YzSeries || '',item.YzzSeries || '',item.QtSeries || '',item.nonum || ''
    ])
  })
