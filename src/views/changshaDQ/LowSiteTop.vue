<template>
    <div class='Low'>
        ...
        <el-table class="table" ref="table" border :data="currentData" v-loading="dataListLoading"
            ...
            <!-- <el-table-column :show-overflow-tooltip="true" align="center" label="上期数据" /> -->
        </el-table>
...
export default {
    ...
    mounted() {
        ...
        this.getdataListPer()
        // this.getdataupList()
        // console.log(this.getLastMonthSameDate('2024-01-31'));
    },
    methods: {
        exportData() {
        ...
        getLastMonthSameDate(currentDate) {
            // 创建一个新的日期对象  
            const date = new Date(currentDate);

            // 获取当前的年份和月份  
            const year = date.getFullYear();
            const month = date.getMonth(); // 月份从0开始，所以1月是0，2月是1，依此类推  
            ...
            // 判断传入的日期是否是当前月份的最后一天  
            if (date.getDate() === currentMonthDays) {
                // 设置为上一个月的最后一天  
                date.setMonth(month - 1);


                // 如果设置的月份是负数（即1月变成了0月），需要调整年份  
                if (date.getMonth() < 0) { // 0表示1月  
                ...
                date.setDate(lastMonthDays); // 设置为上一个月的最后一天  
            } else {
                // 设置为上个月的同一天  
                date.setMonth(month - 1);
                ...
                // 如果当前日期大于上一个月的天数，则将日期设置为上一个月的最后一天  
        ...
        getdataupList() {
            ...
            this.dataForm1.p_vouchdatestart = this.getLastMonthSameDate(this.dataForm.p_vouchdatestart)
            this.dataForm1.p_vouchdateend = this.getLastMonthSameDate(this.dataForm.p_vouchdateend)
            api.companylowApi(this.dataForm1).then(res => {
                this.dataList1 = res
                this.dataList1.forEach(item => {
                    item.iParentName = item.iParentName.replace(/\d+/g, '').trim()
                    item.box = Number(item.box).toFixed(0)
                })
                this.dataList1.sort((a, b) => Number(b.box) - Number(a.box));
                console.log(this.dataList1, 123456)
        ...
        getdataList() {
            ...
            api.companylowApi(this.dataForm).then(res => {
                ...
                this.dataList.sort((a, b) => Number(b.box) - Number(a.box));
