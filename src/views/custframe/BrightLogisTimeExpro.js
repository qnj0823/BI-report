...
export function exportExcel(tableList, fileName) {
  let tableData = [
    ['区域', '光明站点code', '光明站点',  '光明产品编号','光明产品名称','第几天到货','sheet编号','特殊到货规则',]
  ]
  tableList.forEach(item => {
    tableData.push([item.areaName || '', item.wlSiteCode || '', item.wlSiteName || '',item.factoryProductCode || '',item.factoryProductName || '',item.days || '',item.sheets || '',item.dates || ''
    ])
  })
