  ...
  <el-dialog :title="!dataForm.id ? '新增-ADD' : '修改-EDITE'" :close-on-click-modal="false" :visible.sync="visible">
    ...
    <el-form :model="dataForm" ref="dataForm" @keyup.enter.native="dataSubmit()" label-width="160px">
      ...
      <el-form-item label="产品名称" prop="fgsbscName">
        <!-- 选择 -->
        <!-- <el-input style="width: 20rem; " v-model="dataForm2.box" placeholder="搜索产品名称" @input="onInputChange"></el-input> -->
      ...
      <el-form-item label="换算单位（小件/大件）" prop="fgsbscName">
        <el-input v-model="dataForm3.converBigPiece" placeholder="换算单位（小件/大件）"></el-input>
      </el-form-item>
    ...
    dataSubmit() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          api.BdtrackProduct(this.dataForm3).then(res => {
            // TODO 保存数据
