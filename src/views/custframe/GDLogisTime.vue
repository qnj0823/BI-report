    ...
    methods: {
        ...
        async HandleuploadFile() {
            ...
            if (this.dataForm2.file) {
                ...
                const res = await api.importTestAPi(formData)
                this.testList = res
        ...
        HandleuploadFiletest() {
            if (this.types == '覆盖') {
                this.uploadFile('importproCycleGDAPi')
            } else if (this.types == '追加') {
                this.uploadFile('importproCycleaddGDAPi')
            }
        },
        //导入
        uploadFile(API) {
        ...
        exportData() {
            this.$confirm('是否导出表格数据到Excel?', '提示', {
                ...
                exportExcel(this.dataList, '光明站点产品到货周期表(广东).xlsx')
        ...
        deleteHandle(id) {
            ...
            this.$confirm(`您确定删除该条数据吗？`, '提示', {
                ...
                console.log('删除', ids);
                api.wlFacteprodelDGApi(ids).then(res => {
                    // TODO 处理删除
