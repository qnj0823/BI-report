    ...
    <el-dialog :title="!dataForm.id ? '新增-ADD' : '修改-EDITE'" :close-on-click-modal="false" :visible.sync="visible">
        ...
        <el-form :model="dataForm" ref="dataForm" @keyup.enter.native="dataSubmit()" label-width="160px">
            ...
            <el-form-item label="规格" prop="fgsbscName">
                <el-input v-model="dataForm.specifications" style="width: 25rem;" disabled placeholder="规格">
                </el-input>
            </el-form-item>
...
export default {
    data() {
        return {
            ...
            dataForm: {
                id: 0,//主键
                factoryCode: "",//	所属光明工厂编码
                factoryName: '',//所属光明工厂
                factoryProductCode: '',//光明产品编号
                factoryProductName: "",//光明产品名称
                specifications: '',//	规格
            },
            dictForm: {
                page: 0,
    ...
    methods: {
        ...
        handlebigChange(value) {
            const selectedItem = this.FactoryList.find(item => item.value === value);
            this.selectedminLabel = selectedItem.label;
            this.dataForm.factoryProductCode = this.selectedbigValue//光明产品编号
            this.dataForm.factoryProductName = this.selectedminLabel
            this.dataForm.specifications = selectedItem.dictSort
            console.log(this.selectedminLabel)
        },
        ...
        init(id, data) { // 初始化表单验证规则
            ...
            this.visible = true
            this.$nextTick(() => {
                this.selectedbigValue = data.factoryProductCode
                ...
                this.dataForm.specifications = data.specifications
        ...
        dataSubmit() {
            this.$refs['dataForm'].validate((valid) => {
                if (!this.dataForm.id) {
                    api.wlProductaddApi(this.dataForm).then(res => {
                        // TODO 保存数据
                        this.$emit('refreshDataList')
                        this.$emit('close')
                    ...
                    api.wlProductupdataApi(this.dataForm).then(res => {
                        // TODO 保存数据
                        this.$emit('refreshDataList')
                        this.$emit('close')
