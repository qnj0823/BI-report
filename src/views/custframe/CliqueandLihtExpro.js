...
export function exportExcel(tableList, fileName) {
  let tableData = [
    ['地区分类', '集团销售产品编号', '集团销售产品名称', '规格','光明产品编号','光明产品名称','光明产品简称','规格','光明工厂']
  ]
  tableList.forEach(item => {
    tableData.push([item.areaName || '', item.jtProductCode || '', item.jtProductName || '', item.jtSpecifications || '', item
      .factoryProductCode || '', item.factoryProductName || '', item.factoryProductSimpleName || '', item.factorySpecifications || '', item.factoryName || ''
    ])
  })
