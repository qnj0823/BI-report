<template>
    ...
    <el-dialog :title="!dataForm.id ? '新增-ADD' : '修改-EDITE'" :close-on-click-modal="false" :visible.sync="visible">
        ...
        <el-form :model="dataForm" ref="dataForm" @keyup.enter.native="dataSubmit()" label-width="160px">
            ...
            <el-form-item label="光明站点code" prop="sitecode">
                <el-input v-model="dataForm.sitecode" style="width: 100%;" placeholder="光明站点code">
                </el-input>
            </el-form-item>
            <el-form-item label="天数" prop="areaname">
                <el-input v-model="dataForm.days" style="width: 100%;" placeholder="销售区域">
                </el-input>
            </el-form-item>
        </el-form>
    ...
    methods: {
        ...
        dataSubmit() {
            this.$refs['dataForm'].validate((valid) => {
                if (!this.dataForm.id) {
                    api.BDwldateaddApi(this.dataForm).then(res => {
                        // TODO 保存数据
                        this.$emit('refreshDataList')
                        this.$emit('close')
                    ...
                    api.BDwldateupdateApi(this.dataForm).then(res => {
                        // TODO 保存数据
                        this.$emit('refreshDataList')
                        this.$emit('close')
