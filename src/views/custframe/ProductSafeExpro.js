...
export function exportExcel(tableList, fileName) {
  let tableData = [
    ['大类', '小类', '产品简称', '产品名称', '产品编号', '换算单位（箱/盒）', '换算单位（小件/大件）', '属性', '密度系数', '件换算值(KG)', '备注']
  ]
  tableList.forEach(item => {
    tableData.push([item.productClassPname || '', item.productClassName || '', item.simplename || '', item.name || '', item
      .code || '', item.converBox || '', item.converBigPiece || '', item.attribute || '', item.bulkfactor || '', item.pieceConverValue || '', item.def18Name || ''
    ])
  })
